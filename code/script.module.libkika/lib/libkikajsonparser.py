...
def getBrands():#Shows
	...
	for entry in j['_embedded']['items']:
		...
		d['thumb'] = entry.get('mediumLogoImageUrl', entry.get('smallLogoImageUrl', entry.get('largeLogoImageUrl', None)))
		d['_fanart'] = entry.get('mediumBackgroundImageUrl', entry.get('smallBackgroundImageUrl', entry.get('largeBackgroundImageUrl', None)))
...
def getVideos(uri='/api/videos?limit=20&orderBy=date&orderDirection=DESC'):
	...
	for entry in j['_embedded']['items']:
		d = {}
		d['name'] = entry['title']
		brand = deep_get(entry, '_embedded.brand')
		if brand:
			d['name'] = brand['title'] + ' - ' + d['name']
			d['_fanart'] = brand.get('mediumBackgroundImageUrl', brand.get('smallBackgroundImageUrl', brand.get('largeBackgroundImageUrl', None)))
		if 'description' in entry:
			d['plot'] = entry['description']
		d['_duration'] = str(entry['duration'])
		d['thumb'] = entry.get('mediumTeaserImageUrl', entry.get('smallTeaserImageUrl', entry.get('largeTeaserImageUrl', None)))
